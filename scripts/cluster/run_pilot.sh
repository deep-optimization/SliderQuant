#!/usr/bin/env bash
# Apply the complete soft/hard schedule to a balanced AYOT subset.
source "$(dirname "$0")/preflight.sh"

RUN_DIR="$SCALEQ_ROOT/runs/pilot-$SCALEQ_RUN_ID"
if complete_for_current_source "$RUN_DIR"; then
    exit 0
fi
require_complete "$AYOT_DIR"
require_complete "$SCALEQ_ROOT/runs/smoke-$SCALEQ_RUN_ID"
mkdir -p "$RUN_DIR"
resume_args=()
...
"$TORCHRUN_BIN" --standalone --nproc_per_node="$SCALEQ_GPU_COUNT" main.py \
    ...
    --calib_subset "$AYOT_DIR/subset-128.json" \
    --nsamples 128 \
    --output_dir "$RUN_DIR" \
    --cache_dir "$CACHE_DIR" \
    "${resume_args[@]}" \
