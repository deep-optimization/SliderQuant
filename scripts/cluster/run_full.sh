#!/usr/bin/env bash
# Optimize against the complete 4M-token calibration artifact.
source "$(dirname "$0")/preflight.sh"

RUN_DIR="$SCALEQ_ROOT/runs/full-$SCALEQ_RUN_ID"
if complete_for_current_source "$RUN_DIR"; then
    exit 0
fi
require_complete "$AYOT_DIR"
require_complete "$SCALEQ_ROOT/runs/pilot-$SCALEQ_RUN_ID"
mkdir -p "$RUN_DIR"
resume_args=()
